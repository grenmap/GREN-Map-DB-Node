...
identify elements in the map database to which Actions may be applied.
This file defines the base class that all match types should subclass.
"""
...
#     """"
#     This magic MatchType uses a combination of psychic powers and
#     artificial intelligence to select the desired element.
#     """"
# ))
class BaseMatchType:
    """
    Prototype of a class for which the main purpose is to filter a
    list of elements by various criteria, defined by associated
    MatchCriteria and their MatchInfos.  Matched elements may
    ultimately be passed to a subclass of BaseActionType.  Sets of
    matches and actions comprise Rules.
    ...
    *** This class should not be instantiated directly. ***
    ...
    # Input field names required in order to apply this type of match
    # Set to an empty list ("[]") if no info is required.
    required_info = None

    # Input field names that may further adjust behaviour of the match
    # but are not strictly required.
    optional_info = []

    def __init__(self, match_criterion):
        """
        Accepts the MatchCriterion that usually would spawn an
        ...
        if (not self.name) or (not self.element_type) or (self.required_info is None):
            raise NotImplementedError(
                'BaseMatchType subclass {} must specify all required properties.'.format(
    ...
    def filter(self, elements_query):
        ...
        Accepts a QuerySet (elements_query parameter) and further
        filters it.
        """
        raise NotImplementedError(
            'BaseMatchType subclass {} must override the filter method.'.format(
                self.__class__.__name__,
            )
        )
    ...
    def validate_input(self, input_tuple_list):
        """
        Accepts a list of tuples provided as MatchInfo input.

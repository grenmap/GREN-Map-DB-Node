...
DELETE_INSTITUTION_CLASS_NAME = 'DeleteInstitution'
DELETE_INSTITUTION_ACTION_TYPE_NAME = 'Delete Institution'


@pytest.mark.django_db
class TestDeleteInstitution:
    """
    This class contains tests for the Delete Institution action.
    Relies on the MatchType specified in
    MATCH_INSTITUTION_BY_ID_CLASS_NAME.
    """
    @pytest.fixture
    ...
    def test_build_rule(self, load_match_and_action_types, two_nodes_and_links, simple_rule):
        ...
        """
        delete_inst_action_type = ActionType.objects.get(
            class_name=DELETE_INSTITUTION_CLASS_NAME,
        )
        delete_inst_action = Action.objects.create(
            rule=simple_rule,
            action_type=delete_inst_action_type,
        )
        assert delete_inst_action.action_type.name == DELETE_INSTITUTION_ACTION_TYPE_NAME

    @pytest.mark.skip(reason='Currently no MatchType to match all elements of a particular type.')
    def test_delete_all_institutions(
        self,
        ...
        by two Links, each owned by an Institution.  Applies a Rule
        with the DeleteInstitution Action, then verifies that the
        database retains the Nodes and Links but has deleted the
        Institution.
        """
        delete_inst_action_type = ActionType.objects.get(
            class_name=DELETE_INSTITUTION_CLASS_NAME,

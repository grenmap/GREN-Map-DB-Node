...
DELETE_NODE_TAG_PROPERTY_CLASS_NAME = 'DeleteNodeTagProperty'
DELETE_NODE_TAG_PROPERTY_ACTION_TYPE_NAME = 'Delete Node Tag Property'


@pytest.mark.django_db
class TestDeleteNodeTagProperty:
    ...
    def two_nodes_and_links(self):
        ...
        Populates the database with a few GRENML records for testing:
            ...
            - two nodes,
            - two links, and
            - one property of type 'tag' in the node
        """
    ...
    def institution_with_ten_properties_five_same_name(self):
        ...
        Populates the database with a few GRENML records for testing:
            - one node,
            - ten properties of type 'tag' in the node,
    ...
    def test_build_rule(self, load_match_and_action_types, two_nodes_and_links, simple_rule):
        ...
        """
        delete_node_tag_prop_action_type = ActionType.objects.get(
            class_name=DELETE_NODE_TAG_PROPERTY_CLASS_NAME,
        )
        delete_node_tag_prop_action = Action.objects.create(
            rule=simple_rule,
            action_type=delete_node_tag_prop_action_type,
        )
        assert (
            delete_node_tag_prop_action.action_type.name
            == DELETE_NODE_TAG_PROPERTY_ACTION_TYPE_NAME
        )
    ...
    def test_affected_element_counts(
        ...
        ActionInfo.objects.create(
            action=delete_node_tag_prop_action,
            key='value',
            value='Test'
        )
    ...
    def test_same_names(
        ...
        target_property = grenml.Property.objects.all().filter(
            property_for=target_node, name='tag', value='Value_2')
    ...
    def test_no_corresponding_property(
        ...
        target_node = grenml.Node.objects.all().filter(
            grenml_id=target_node_id)[0]
